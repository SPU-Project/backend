...
//Create
const addStokBahanBaku = async (req, res) => {
  const getUserInfo = async (req) => {
    if (!req.session.userId) return null;
    const user = await Admin.findOne({
      attributes: ["username", "role"],
  ...
  try {
    ...
    // Create a new record in the StokBahanBaku
    const newStokBahanBaku = await StokBahanBaku.create({
      Stok,
      TanggalPembaruan,
    });
    ...
    res.status(201).json({
      ...
      data: newStokBahanBaku,
    });
  } catch (error) {
    res.status(400).json({
      message: "Gagal Menambahkan Stok Bahan Baku",
      error: error.message,
    });
  }
};
